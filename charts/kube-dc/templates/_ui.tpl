...
{{- dict "schemaVersion" 1 "application" .application "environment" $r.Values.ui.environment "environmentLabel" $r.Values.ui.environmentLabel "basePath" "/" "apiBaseUrl" "/api/ui/v1" "keycloakBaseUrl" $r.Values.manager.keycloakSecret.url "clientId" (ternary "kube-dc-admin-console" "kube-dc" $admin) "defaultOrganization" $r.Values.ui.defaultOrganization "consoleUrl" $console "adminUrl" $adminUrl "buildLabel" $r.Values.ui.buildLabel "partnerApiBaseUrl" (printf "https://%s/api/public/v1" $r.Values.backend.gateway.hostname) "kubeDcDomain" (trimPrefix "console." $r.Values.frontend.gateway.hostname) "managedServices" $r.Values.ui.managedServices "ssoEnabled" $sso "googleSsoEnabled" $google "ssoClientId" "kube-dc" | toJson -}}
{{- end -}}
