...
func LoadDNS01Route53(zoneID, region, accessKeyID, secretKeyFile, secretKeyEnv string) (*DNS01Route53Material, error) {
	...
	case secretKeyFile != "":
		...
		if err != nil {
			// Never echo the path: an operator who pastes the KEY where the
			// path belongs would otherwise see it in stderr / CI logs.
			return nil, fmt.Errorf("read --dns01-route53-secret-key-file: %w (path withheld)", sanitizedReadErr(err))
		}
		raw = b
	case secretKeyEnv != "":
...
// dns01SecretSetKeys are the secret-shaped keys that must never arrive via
// --set: Sets are echoed in the options summary and persisted to
// cluster-config.env in cleartext. The prefill deny-list (prefill.go) covers
// config FILES; this covers the command line (codex review 2026-08-31, P1).
var dns01SecretSetKeys = []string{
	"DNS01_ROUTE53_SECRET_KEY", "DNS01_ROUTE53_SECRET_ACCESS_KEY",
	"DNS01_CLOUDFLARE_API_TOKEN", "CLOUDFLARE_API_TOKEN",
}

func validateDNS01Flags(o *InitOptions) error {
	for _, k := range dns01SecretSetKeys {
		if _, ok := o.Sets[k]; ok {
			return fmt.Errorf("--set %s is refused — secret material never rides --set (it would be logged and persisted in cleartext); use the file flag or the environment variable", k)
		}
	}
	for _, k := range promotedTLSKeys {
		if _, ok := o.Sets[k]; ok {
			return fmt.Errorf("--set %s is not accepted — use --tls-mode / --dns01-route53-* / --dns01-cloudflare-* flags (dedicated, validated fields)", k)
		}
	}
	mode := canonicalTLSMode(o.TLSMode)
	r53Mode := mode == TLSModeACMEDNS01Route53
	cfMode := mode == TLSModeACMEDNS01Cloudflare
	r53Any := o.DNS01Route53ZoneID != "" || o.DNS01Route53Region != "" ||
		o.DNS01Route53AccessKeyID != "" || o.DNS01Route53SecretKeyFile != ""
	cfAny := o.DNS01CloudflareZone != "" || o.DNS01CloudflareAPITokenFile != ""

	// One DNS-01 provider per cluster: both writers patch the same
	// ClusterIssuer solvers list, and whichever kustomize applied last would
	// silently win.
	if r53Any && cfAny {
		return fmt.Errorf("--dns01-route53-* and --dns01-cloudflare-* are mutually exclusive — one DNS-01 provider per cluster")
	}
	if cfMode && r53Any {
		return fmt.Errorf("--dns01-route53-* flags are not valid with --tls-mode=%s", TLSModeACMEDNS01Cloudflare)
	}
	if r53Mode && cfAny {
		return fmt.Errorf("--dns01-cloudflare-* flags are not valid with --tls-mode=%s", TLSModeACMEDNS01Route53)
	}
	// Cloudflare flags ride two modes: the dedicated one (every certificate)
	// and plain acme (wildcards only). byo-wildcard has no ACME issuer to
	// patch. The token itself is checked by LoadDNS01Cloudflare in the cmd
	// layer — it may legitimately arrive only via environment at apply time.
	if cfAny && !cfMode && mode != "" {
		return fmt.Errorf("--dns01-cloudflare-* flags are only valid with --tls-mode=%s or --tls-mode=%s (wildcards only)", TLSModeACMEDNS01Cloudflare, TLSModeACME)
	}
	if !r53Mode {
		if r53Any {
...
// dns01ProviderMarkers lists every DNS-01 solver block a kube-dc writer can
// leave in platform.yaml. A writer refuses a file carrying ANOTHER provider's
// block: both patch the same ClusterIssuer solvers list.
var dns01ProviderMarkers = []string{dns01Route53Marker, dns01CloudflareMarker}

// dns01Route53BlockHeader is the comment header of the Route53 owned block.
const dns01Route53BlockHeader = "    # The default issuer solves HTTP-01 through the Gateway, which requires\n" +
...
func patchPlatformDNS01Solver(entries string) func([]string) ([]string, bool, error) {
	return patchPlatformDNS01SolverWith(dns01Route53Marker, dns01Route53BlockHeader, entries)
}

// patchPlatformDNS01SolverWith is the provider-neutral writer behind
// patchPlatformDNS01Solver: marker identifies the owned block, header is its
// comment body (may vary with configuration — it is part of the content
// comparison), entries is the rendered spec.patches list entry.
func patchPlatformDNS01SolverWith(marker, header, entries string) func([]string) ([]string, bool, error) {
	return func(lines []string) ([]string, bool, error) {
		hasPatchesKey := false
		markerAt := -1
		for i, l := range lines {
			t := strings.TrimSpace(l)
			if markerAt < 0 && strings.Contains(t, marker) {
				markerAt = i
			}
			for _, other := range dns01ProviderMarkers {
				if other != marker && strings.Contains(t, other) {
					return nil, false, fmt.Errorf("platform.yaml already carries a different DNS-01 provider's solver block (%q) — remove it first; one DNS-01 provider per cluster", other)
				}
			}
			if t == "patches:" {
				hasPatchesKey = true
		...
		newBlock := strings.Split(dns01BlockBody(marker, header, entries), "\n")
		if markerAt >= 0 {
			end, err := dns01OwnedBlockEnd(lines, markerAt, marker)
			if err != nil {
		...
		if hasPatchesKey {
			...
			if !ours {
				return nil, false, fmt.Errorf("platform.yaml already has a patches: block (hand-edited?) — add the DNS-01 solver patch manually (docs/platform/certificates.md, marker: %q)", marker)
			}
		}
		...
		block := dns01BlockBody(marker, header, entries)
		if !hasPatchesKey {
			block = "  patches:\n" + block
		}
...
func dns01BlockBody(marker, header, entries string) string {
	return "    # --- " + marker + " ---\n" + header + strings.TrimRight(entries, "\n")
}
...
func dns01OwnedBlockEnd(lines []string, markerAt int, marker string) (int, error) {
	j := markerAt
	for j < len(lines) && strings.HasPrefix(lines[j], "    #") {
		j++
	}
	if j >= len(lines) || !strings.HasPrefix(lines[j], "    - ") {
		return 0, fmt.Errorf("platform.yaml: the DNS-01 marker is present but its patch entry is not where the writer left it — fix the block manually (marker: %q)", marker)
	}
