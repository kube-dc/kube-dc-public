...
func TestParseAnchorMap_Valid(t *testing.T) {
	got, err := ParseAnchorMap("srv7-kub1=100.64.0.13/16,srv5-kub1=100.64.0.11/16,srv6-kub1=100.64.0.12/16")
	if err != nil {
	...
	want := []Entry{
		...
		{Host: "srv7-kub1", CIDR: "100.64.0.13/16"},
	}
	if len(got) != len(want) {
...
func TestParseAnchorMap_ToleratesWhitespace(t *testing.T) {
	got, err := ParseAnchorMap("  srv5-kub1 = 100.64.0.11/16 , srv6-kub1=100.64.0.12/16  ")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if len(got) != 2 {
	...
	if got[0].Host != "srv5-kub1" || got[0].CIDR != "100.64.0.11/16" {
...
func TestParseAnchorMap_Errors(t *testing.T) {
	cases := []struct {
		...
		{"missing equals", "srv5-kub1-100.64.0.11/16", "missing '='"},
		{"empty host", "=100.64.0.11/16", "empty host"},
		{"duplicate host", "srv5-kub1=100.64.0.11/16,srv5-kub1=100.64.0.12/16", "more than once"},
		{"bad CIDR", "srv5-kub1=100.64.0.11", "invalid CIDR"},
		{"bad mask", "srv5-kub1=100.64.0.11/99", "invalid CIDR"},
	}
