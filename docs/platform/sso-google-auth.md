...
Kube-DC supports Google OAuth authentication through a central `sso` Keycloak realm that brokers authentication to organization-specific realms. This allows:
...
2. A `KUBECONFIG` for the cluster (the CLI reads the Keycloak admin password
   from it, and you do not need the password yourself)
...
3. Enter a project name (for example, `kube-dc-sso`)
4. Click **Create**
...
> self-registration and verification. The console then shows "Sign up with
...
realm exists. That order matters, because the flag alone makes
...
#### Option A: GitOps fleet (the recommended, supported path)
