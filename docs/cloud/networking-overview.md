...
This page explains how Kube-DC networking works: how your Project connects to
the internet, how traffic flows, and which tools expose your services.

## Key concepts

| Resource | What It Does |
|----------|-------------|
| **VPC** | Isolated virtual network for your Project. Every VM and pod gets a private IP here |
...
### Inbound through Gateway route (HTTPS)
...
Kube-DC Cloud Project networks use an encapsulated overlay, so the usable MTU inside a project is **1400 bytes**, not the 1500 you may be used to. Kube-DC configures your VMs and pods with this value. A VM's interface picks up 1400 over DHCP, and pods get it from the network plugin. You normally never need to think about it.
...
**The exception is software that assumes 1500 and does not inherit the MTU from its host. Docker is the most common case.** If you install Docker inside a VM, Docker creates its default bridge with an MTU of 1500. Containers on that bridge then send packets too large for the 1400 network, and those packets are silently dropped.
...
It looks like a broken network, but connectivity is fine. Only the oversized packets are lost. `docker pull`, `apt-get`, and `npm install` can fail the same way.
...
| Result | Cause | Fix |
|--------|-------|-----|
| Works on the VM, fails in Docker | Docker's container MTU | See [Fix: set Docker's MTU](#fix-set-dockers-mtu) |
| Fails on the VM too | The VM's own interface | Run `ip link show`. It must report `mtu 1400` |
...
If you build with BuildKit or `docker buildx`, recreate the builder after you change the daemon config, so that it picks up the new MTU. As an alternative, build with `--network=host`, so that the build steps use the VM's interface directly.
...
## Manage networking in the console
...
- [External and floating IPs](public-floating-ips.md): Create and manage EIPs and FIPs
- [VPC and private networking](private-networking.md): Understand project isolation and subnets
- [Datacenter VLANs](datacenter-vlans.md): Put workloads on a physical network segment to reach your own hardware
- [Service exposure](service-exposure.md): Complete reference for all exposure methods with examples
