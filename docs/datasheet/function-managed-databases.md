...
**Managed PostgreSQL and MariaDB for every team — without each tenant
operating a database engine.**

Tenants provision databases the way they provision everything else on the
platform: a short manifest or a console form. The platform's controllers
run the engines, replication and backups; teams get an endpoint and a
credentials Secret.

## 1. Engines and provisioning
...
## 7. Responsibilities — managed databases
